...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Settling a thread also removes the git worktree it owned, when no other
  // thread points at that path. Off by default: settling is reversible, the
  // worktree removal is not.
  deleteWorktreeOnSettle: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
...
export const ServerSettings = Schema.Struct({
  ...
  // Create new worktrees under `<project>/.t3/worktrees` instead of the
  // shared worktrees directory in T3 home.
  worktreesInsideProject: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  worktreesInsideProject: Schema.optionalKey(Schema.Boolean),
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  deleteWorktreeOnSettle: Schema.optionalKey(Schema.Boolean),
